...
class HIVEnv(gym.Env):
    def __init__(self, state_with_time) -> None:
        super().__init__()
        self.sim = HIVSimulator(podmp=True)
        self.state_with_time = state_with_time
        self.time = 0
        if state_with_time:
            self.observation_space = gym.spaces.Box(-np.inf, np.inf, (5, ))
    ...
    def reset(self):
        self.time = 0
        return np.concatenate([[self.time/1000.0], self.sim.reset()])

    def step(self, action):
        dt = self.sim.get_time_gap(action)
        return np.concatenate([[self.time/1000.0], self.sim.step(dt)])

    def get_time_info(self):
        return self.sim.get_time_info()


def make_hivsim_env(state_with_time):
    return HIVEnv(state_with_time)
